...
void SplitLR ( u32bit &L, u32bit &R, const u64bit x )
{
	u64bit temp = x;
	R = temp & 0xffffffff;
	temp >>= 32;
...
// P-блок 
void ExpandPermute ( u64bit &out, const u32bit x)
{
	u64bit y = 0;
	u32bit temp;
...
void ParDropPermute ( u64bit &out, const u64bit x )
{
	u64bit y = 0;
	u64bit temp;
...
void SplitKeyCD ( u32bit &C, u32bit &D, const u64bit x )
{
	u64bit temp = x;
	D = temp & 0xfffffff;
	temp >>= 28;
...
void Encrypt ( u64bit &TC, u64bit TO )
{
...
void Decrypt ( u64bit &TD, u64bit TC )
{
	// u64bit RoundKey[16]  =	{0};	
