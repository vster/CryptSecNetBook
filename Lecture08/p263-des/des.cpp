...
void KeyGen ( const byte key[8] )
{
	...
	Key = make_u64bit( key[0],key[1],key[2],key[3], 
...
void EncryptDES ( const byte in[BS], byte out[BS] )
{
	u64bit TO;
	u64bit TC;

	TO = make_u64bit(	in[0], in[1], in[2], in[3],
						in[4], in[5], in[6], in[7]);
...
void DecryptDES ( const byte in[BS], byte out[BS] )
{
	u64bit TC; 
	u64bit TD;
	
	TC = make_u64bit(	in[0], in[1], in[2], in[3],
						in[4], in[5], in[6], in[7]);
	// u64bit RoundKey[16]  =	{0};	
