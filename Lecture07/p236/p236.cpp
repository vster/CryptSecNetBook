...
void GenRoundKeys ( byte w[78][4], byte Key[] )
{
	for ( int i = 0; i < Nk; i++ )
		for ( int j = 0; j < 4; j++ )
			w[i][j] = Key[4*i + j];
	...
	{
		byte t[4];		
		GenT ( t, w[Nk*round-1], round );
		
		for ( int j = 0; j < Nk; j++ )
			w[Nk*round][j] = t[j]^w[Nk*round-Nk][j];

 		for ( int i = 1; i < Nk; i++ )
			for ( int j = 0; j < 4; j++ )
				w[Nk*round+i][j] = w[Nk*round+i-1][j]^w[Nk*round+i-Nk][j];
		
		
		cout << setw(2) << dec << round << " ";
		for ( int j = 0; j < 4; j++ )
			cout << hex << setw(2) << setfill ('0') << int (t[j]);
		cout << " ";
...
int main()
{
	...
	const int KLB = KeyLen/8;
	byte Key[KLB];

	for ( int i = 0; i < KLB; i++ )
	 	Key[i] = i;

	const long Nw = Nk*(Nr+1);
	byte w[Nw][4];
